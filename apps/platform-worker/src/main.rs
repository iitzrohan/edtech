//! Composition root for the future Platform worker process.
//!
//! Checkpoint 1 contains configuration, logging, supervision, and signal handling only: no
//! database, broker, identity-provider, cache, or domain behavior belongs here.
...
async fn main() -> Result<()> {
    ...
    info!(service = service.as_str(), environment = %config.base().environment(), "process started");
